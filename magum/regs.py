...
A_CTRL_REG2 = 0x2b  # st_bit,rst_bit,Accelerometer Sleep mode,OSR mode selection,Auto-sleep mode,Accelerometer Wake
# mode OSR mode selection
...
# Auto-Sleep trigger
A_ASPL_COUNT = 0x29  # ASPL_COUNT register: sets the minimum time period of event flag inactivity required to initiate
#  a change from the current active mode ODR value specified in CTRL_REG1[dr] to the Sleep mode ODR value specified in
#  CTRL_REG1[aslp_rate], provided that CTRL_REG2[slpe] = 1
...
A_VECM_CFG = 0x5f  # latch enable mode, set initial reference values, reference values updating mode, accelerometer
# vector-magnitude function enable/disable
A_VECM_THS_MSB = 0x60
A_VECM_THS_LSB = 0x61
A_VECM_CNT = 0x62
...
A_TRANSIENT_CFG = 0x1d  # Transient event flag latch enable, Z-axis transient event flag enable, Y-axis transient
# event flag enable, X-axis transient event flag enable, Transient function high-pass filter bypass enable
...
G_TEMP = 0x12  # contains an 8-bit 2's complement temperature value with a range of –128 °C to +127 °C and a scaling
# of 1 °C/LSB
...
G_CTRL_REG3 = 0x15  # enables FSR expansion, ext. power control input, options to modify the auto-increment read
# address pointer behavior when doing burst reads of the FIFO data
...
COMPLETE_REGS_DICT = {'A_TRIG_CFG': A_TRIG_CFG, 'A_CTRL_REG1': A_CTRL_REG1, 'A_CTRL_REG2': A_CTRL_REG2,
                      'A_CTRL_REG3': A_CTRL_REG3, 'A_CTRL_REG4': A_CTRL_REG4, 'A_CTRL_REG5': A_CTRL_REG5,
                      'A_ASPL_COUNT': A_ASPL_COUNT, 'A_F_SETUP': A_F_SETUP, 'A_XYZ_DATA_CFG': A_XYZ_DATA_CFG,
                      'A_HP_FILTER_CUTOFF': A_HP_FILTER_CUTOFF, 'A_PL_CFG': A_PL_CFG,
                      'A_PL_COUNT': A_PL_COUNT, 'A_PL_BF_ZCOMP': A_PL_BF_ZCOMP, 'A_PL_THS_REG': A_PL_THS_REG,
                      'A_FFMT_CFG': A_FFMT_CFG, 'A_FFMT_THS': A_FFMT_THS, 'A_FFMT_COUNT': A_FFMT_COUNT,
                      'A_VECM_CFG': A_VECM_CFG, 'A_VECM_THS_MSB': A_VECM_THS_MSB, 'A_TRANSIENT_CFG': A_TRANSIENT_CFG,
                      'A_TRANSIENT_THS': A_TRANSIENT_THS, 'A_TRANSIENT_COUNT': A_TRANSIENT_COUNT,
                      'A_PULSE_CFG': A_PULSE_CFG, 'A_PULSE_TMLT': A_PULSE_TMLT, 'A_PULSE_LTCY': A_PULSE_LTCY,
                      'A_OFF_X': A_OFF_X, 'A_OFF_Y': A_OFF_Y, 'A_OFF_Z': A_OFF_Z, 'M_OFF_X_MSB': M_OFF_X_MSB,
                      'M_OFF_X_LSB': M_OFF_X_LSB, 'M_OFF_Y_MSB': M_OFF_Y_MSB, 'M_OFF_Y_LSB': M_OFF_Y_LSB,
                      'M_OFF_Z_MSB': M_OFF_Z_MSB, 'M_OFF_Z_LSB': M_OFF_Z_LSB, 'M_THS_CFG': M_THS_CFG,
                      'M_THS_COUNT': M_THS_COUNT,
                      'M_CTRL_REG1': M_CTRL_REG1, 'M_CTRL_REG2': M_CTRL_REG2, 'M_CTRL_REG3': M_CTRL_REG3,
                      'M_VECM_CFG': M_VECM_CFG,
                      'M_VECM_THS_MSB': M_VECM_THS_MSB, 'M_VECM_THS_LSB': M_VECM_THS_LSB, 'M_VECM_CNT': M_VECM_CNT,
                      'G_F_SETUP': G_F_SETUP,
...
A_CREGS_LIST = ['A_TRIG_CFG', 'A_CTRL_REG1', 'A_CTRL_REG2', 'A_CTRL_REG3', 'A_CTRL_REG4', 'A_CTRL_REG5', 'A_ASPL_COUNT',
                'A_F_SETUP', 'A_XYZ_DATA_CFG', 'A_HP_FILTER_CUTOFF', 'A_PL_CFG',
                'A_PL_COUNT', 'A_PL_BF_ZCOMP', 'A_PL_THS_REG', 'A_FFMT_CFG', 'A_FFMT_THS', 'A_FFMT_COUNT', 'A_VECM_CFG',
                'A_VECM_THS_MSB', 'A_TRANSIENT_CFG',
                'A_TRANSIENT_THS', 'A_TRANSIENT_COUNT', 'A_PULSE_CFG', 'A_PULSE_TMLT', 'A_PULSE_LTCY', 'A_OFF_X',
                'A_OFF_Y', 'A_OFF_Z']
M_CREG_LIST = ['M_OFF_X_MSB', 'M_OFF_X_LSB', 'M_OFF_Y_MSB', 'M_OFF_Y_LSB', 'M_OFF_Z_MSB', 'M_OFF_Z_LSB', 'M_THS_CFG',
               'M_THS_COUNT',
               'M_CTRL_REG1', 'M_CTRL_REG2', 'M_CTRL_REG3', 'M_VECM_CFG', 'M_VECM_THS_MSB', 'M_VECM_THS_LSB',
               'M_VECM_CNT']
G_CREG_LIST = ['G_F_SETUP', 'G_CTRL_REG0', 'G_RT_CFG', 'G_RT_THS', 'G_RT_COUNT', 'G_CTRL_REG1', 'G_CTRL_REG2',
               'G_CTRL_REG3']
